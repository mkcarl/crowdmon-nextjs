...
export async function getServerSideProps() {
    ...
    if (dataInCache) {
        ...
        const cropNumberByVideo = allCrops.reduce((acc, crop)=>{
        ...
        const numberOfFramesByVideo = allVideos.reduce((acc, video)=>{
            acc[video.name] = video.frames.length
            return acc
        }, {} as Record<string, number>)

        const cropStatus = Object.entries(cropNumberByVideo).reduce((acc, [videoId, cropNumber])=>{
            acc[videoId] = `${(cropNumber/numberOfFramesByVideo[videoId]*100).toFixed(2)}%`
            return acc
