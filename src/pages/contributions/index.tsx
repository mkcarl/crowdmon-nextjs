...
export const getStaticProps: GetStaticProps<ContributionPageProps> = async (context) => {
    const crops = await getAllCrops()
    const accumulator: Record<string, {contribution:number, lastContribution: number}> = {}

    for (const crop of crops) {
        if (crop.contributor_id in accumulator) {
            accumulator[crop.contributor_id].contribution += 1
            accumulator[crop.contributor_id].lastContribution = Math.max(accumulator[crop.contributor_id].lastContribution, crop.timestamp)
